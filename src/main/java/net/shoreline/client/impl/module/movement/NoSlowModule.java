...
public class NoSlowModule extends ToggleModule
{
    ...
    Config<Boolean> sneakConfig = (new BooleanConfig("Sneak", "Removes sneak slowdown", false));
    Config<Boolean> crawlConfig = (new BooleanConfig("Crawl", "Removes crawl slowdown", false));
    ...
    Config<Float> webSpeedConfig = (new NumberConfig<>("WebMultiplier", "Speed to fall through webs", 0.00f, 1.00f, 1.00f, () -> websConfig.getValue() || berryBushConfig.getValue()));
    ...
    {
        if (event.getStage() == EventStage.POST && grimConfig.getValue()
...
//                              new BlockHitResult(mc.player.getPos(), Direction.UP, mc.player.getBlockPos(), false), id));
    ...
    public void onMovementSlowdown(MovementSlowdownEvent event)
    {
        ...
        {
            float f = 1.0f / (float) mc.player.getAttributeValue(EntityAttributes.GENERIC_MOVEMENT_SPEED);
