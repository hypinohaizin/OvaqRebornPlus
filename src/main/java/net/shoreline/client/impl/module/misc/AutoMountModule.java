...
{
    Config<Float> rangeConfig = (new NumberConfig<>("Range", "The range to mount entities", 0.1f, 4.0f, 6.0f));
    Config<Boolean> forceConfig = (new BooleanConfig("ForceMount", "Forces the mounting without packets", false));
    Config<Boolean> horseConfig = (new BooleanConfig("Horse", "Mounts horses", true));
    Config<Boolean> donkeyConfig = (new BooleanConfig("Donkey", "Mounts donkeys", true));
    Config<Boolean> muleConfig = (new BooleanConfig("Mule", "Mounts mules", true));
    Config<Boolean> llamaConfig = (new BooleanConfig("Llama", "Mounts llamas", false));

    public AutoMountModule()
    {
        super("AutoMount", "Mounts nearby entities", ModuleCategory.MISC);
    }
