...
{
    Config<Boolean> airConfig = (new BooleanConfig("Air", "Removes acceleration while in the air", true));
    Config<Boolean> downwardsConfig = (new BooleanConfig("Downwards", "Removes acceleration while descending", true));
    ...
    {
        if (Modules.SPEED.isEnabled() || FlightModule.getInstance().isEnabled())
        {
            return;
        }
