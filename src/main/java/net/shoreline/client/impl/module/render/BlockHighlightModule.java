...
public class BlockHighlightModule extends ToggleModule
{
    ...
    Config<Float> widthConfig = (new NumberConfig<>("Width", "The line width of the highlight", 1.0f, 1.0f, 5.0f));
    Config<Boolean> fadeConfig = (new BooleanConfig("Fade", "Fades the block highlight", false));
    Config<Integer> fadeTimeConfig = (new NumberConfig<>("Fade-Time", "Timer for the fade", 0, 200, 1000, () -> false));
    ...
    public void onRenderWorld(RenderWorldEvent event)
    {
        ...
        if (fadeConfig.getValue())
        {
            for (Map.Entry<Box, Animation> set : fadeList.entrySet())
            {
                ...
                renderBb(event.getMatrices(), box, Modules.COLORS.getRGB(boxAlpha), Modules.COLORS.getRGB(lineAlpha));
            }
        }
        else if (render != null)
        {
            renderBb(event.getMatrices(), render, Modules.COLORS.getRGB(40), Modules.COLORS.getRGB(145));
        }
