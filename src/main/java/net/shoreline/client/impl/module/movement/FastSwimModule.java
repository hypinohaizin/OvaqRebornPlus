...
{
    Config<SwimMode> modeConfig = (new EnumConfig<>("Mode", "The mode for swimming", SwimMode.VANILLA, SwimMode.values()));
    Config<Float> waterSpeedConfig = (new NumberConfig<>("WaterSpeed", "Base speed for moving through water", 0.1f, 1.0f, 10.0f));
    Config<Float> lavaSpeedConfig = (new NumberConfig<>("LavaSpeed", "Base speed for moving through lava", 0.1f, 1.0f, 10.0f));
    Config<Boolean> verticalConfig = (new BooleanConfig("Vertical", "Allows moving vertically", true));
    Config<Boolean> elytraConfig = (new BooleanConfig("Elytra", "Applies elytra speed when moving through liquids", false));
    Config<Float> elytraSpeedConfig = (new NumberConfig<>("ElytraSpeed", "Base speed for moving through lava", 0.1f, 1.0f, 10.0f, () -> elytraConfig.getValue()));
   // Config<Boolean> depthStriderConfig = (new BooleanConfig("DepthStrider", "Prefers depth strider", false));
