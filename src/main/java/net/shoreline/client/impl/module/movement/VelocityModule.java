...
public class VelocityModule extends ToggleModule
{
    ...
    Config<Float> horizontalConfig = (new NumberConfig<>("Horizontal", "How much horizontal knock-back to take", 0.0f, 0.0f, 100.0f, NumberDisplay.PERCENT, () -> modeConfig.getValue() == VelocityMode.NORMAL || modeConfig.getValue() == VelocityMode.WALLS));
    Config<Float> verticalConfig = (new NumberConfig<>("Vertical", "How much vertical knock-back to take", 0.0f, 0.0f, 100.0f, NumberDisplay.PERCENT, () -> modeConfig.getValue() == VelocityMode.NORMAL || modeConfig.getValue() == VelocityMode.WALLS));
    Config<Boolean> concealConfig = (new BooleanConfig("Conceal", "Fixes velocity on servers with excessive setbacks", false));
    Config<Boolean> wallsAirConfig = (new BooleanConfig("GroundOnly", "Only applies velocity in walls while on ground", false, () -> modeConfig.getValue() == VelocityMode.WALLS));
    Config<Boolean> wallsTrappedConfig = (new BooleanConfig("Trapped", "Applies velocity while player head is trapped", false, () -> modeConfig.getValue() == VelocityMode.WALLS));
