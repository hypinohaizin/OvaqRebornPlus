...
{
    Config<Boolean> noSwitchConfig = (new BooleanConfig("NoSwitchAnimation", "Removes the animation when switching items", false));
    Config<Boolean> oldSwingConfig =(new BooleanConfig("OldSwingAnimation", "Reverts to the 1.8 swinging animations", false));
    Config<Boolean> swingSpeedConfig = (new BooleanConfig("SwingSpeed", "Allows you to modify your swing speed.", false));
    Config<Integer> swingFactorConfig = (new NumberConfig<>("SwingFactor", "The speed of your swing.", 1, 6, 20, () -> swingSpeedConfig.getValue()));
    Config<Boolean> selfOnlyConfig = (new BooleanConfig("SelfOnly", "Make the module only affect yourself", true, () -> false));
    Config<Boolean> eatTransformConfig = (new BooleanConfig("EatTransform", "Transforms the first person eating animation", false));
    Config<Float> eatTransformFactorConfig = (new NumberConfig<>("EatTransform-Factor", "Factor for the first person eating animation", 0.0f, 1.0f, 1.0f, () -> eatTransformConfig.getValue()));
    Config<Boolean> limbSwing = (new BooleanConfig("NoLimbSwing", "Allows you to cancel limb swing animations", false));
    Config<Boolean> interpolationConfig = (new BooleanConfig("NoInterpolation", "Entities will be rendered at their server positions", false, () -> limbSwing.getValue()));
