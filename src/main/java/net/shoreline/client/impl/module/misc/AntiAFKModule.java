...
{
    //
    Config<Float> secondsConfig = (new NumberConfig<>("AFKTime", "The time before attempting actions", 5.0f, 20.0f, 120.0f));
    ...
    Config<Boolean> jumpConfig = (new BooleanConfig("Jump", "Jumps to prevent AFK kick", true));
    Config<Boolean> autoReplyConfig = (new BooleanConfig("AutoReply", "Replies to players messaging you in chat", false));
    Config<String> replyConfig = (new StringConfig("Reply", "The reply message for AutoReply", "I am currently AFK.", () -> autoReplyConfig.getValue()));
    Config<Float> delayConfig = (new NumberConfig<>("Delay", "The delay between actions", 5.0f, 100.0f, 1000.0f));
    ...
    {
        super("AntiAFK", "Prevents the player from being kicked for AFK", ModuleCategory.MISC);
    }
    ...
    public void onPacketInbound(PacketEvent.Inbound event)
    {
                ...
                && autoReplyConfig.getValue())
        {
            ...
            {
                ChatUtil.serverSendMessage("/r [OvaqPlus] " + replyConfig.getValue());
            }
        }
    }
}
