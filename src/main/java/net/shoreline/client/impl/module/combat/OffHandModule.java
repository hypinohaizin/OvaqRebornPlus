...
{
    private static OffHandModule INSTANCE;

    Config<OffhandItem> itemConfig = (new EnumConfig<>("Item", "The item to wield in your offhand", OffhandItem.TOTEM, OffhandItem.values()));
    Config<Float> healthConfig = (new NumberConfig<>("Health", "The health required to fall below before swapping to a totem", 0.0f, 14.0f, 20.0f));
    ...
    Config<Boolean> mainhandTotemConfig = (new BooleanConfig("MainhandTotem", "Swaps to a totem in your mainhand", false));
    Config<Integer> totemSlotConfig = (new NumberConfig<>("TotemSlot", "Slot to use for mainhand totem", 1, 1, 9, () -> mainhandTotemConfig.getValue()));
    Config<Boolean> alternativeConfig = (new BooleanConfig("Alternative", "Replaces totem using the swap packet", false));
    ...
    public static OffHandModule getInstance()
    {
        return INSTANCE;
    }
    ...
    {
        if (mc.player == null || event.getStage() != EventStage.PRE)
        {
            return;
        }
    ...
    private boolean checkLethalCrystal(float health)
    {
        ...
        for (Entity e : entities)
        {
            ...
            double potential = EndCrystalUtil.getDamageTo(mc.player, crystal.getPos(), false);
