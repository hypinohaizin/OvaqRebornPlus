...
public class ElytraFlyModule extends RotationModule
{
    ...
    // Speeds
    Config<Float> speedConfig = (new NumberConfig<>("Speed", "The horizontal flight speed", 0.1f, 2.5f, 10.0f, () -> modeConfig.getValue() != FlyMode.BOOST && modeConfig.getValue() != FlyMode.BOUNCE));
    Config<Float> vspeedConfig = (new NumberConfig<>("VerticalSpeed", "The vertical flight speed", 0.1f, 1.0f, 5.0f, () -> modeConfig.getValue() != FlyMode.BOOST && modeConfig.getValue() != FlyMode.BOUNCE));
    Config<Boolean> accelerateConfig = (new BooleanConfig("Accelerate", "Accelerates fly speed", false, () -> modeConfig.getValue() != FlyMode.BOOST && modeConfig.getValue() != FlyMode.BOUNCE));
    Config<Float> accelSpeedConfig = (new NumberConfig<>("AccelSpeed", "Acceleration speed", 0.01f, 0.1f, 1.00f, () -> accelerateConfig.getValue() && modeConfig.getValue() != FlyMode.BOOST && modeConfig.getValue() != FlyMode.BOUNCE));
    Config<Float> maxSpeedConfig = (new NumberConfig<>("MaxSpeed", "The maximum flight speed", 0.1f, 3.5f, 10.0f, () -> accelerateConfig.getValue() && modeConfig.getValue() != FlyMode.BOOST && modeConfig.getValue() != FlyMode.BOUNCE));
    Config<Boolean> boostConfig = (new BooleanConfig("VanillaBoost", "Uses vanilla boost speed", false, () -> modeConfig.getValue() != FlyMode.BOOST));
    // Control
    Config<Boolean> rotateConfig = (new BooleanConfig("Rotate", "Rotates the player when moving", false, () -> modeConfig.getValue() == FlyMode.CONTROL));
    Config<Boolean> fireworkConfig = (new BooleanConfig("Fireworks", "Uses fireworks when flying", false, () -> modeConfig.getValue() == FlyMode.CONTROL));
    // Bounce
    Config<Float> pitchConfig = (new NumberConfig<>("Pitch", "The pitch angle of bounce", 70.0f, 80.0f, 90.0f, () -> modeConfig.getValue() == FlyMode.BOUNCE));
    ...
    {
        if (OvaqMod.isBaritonePresent() && BaritoneAPI.getProvider().getPrimaryBaritone().getPathingBehavior().isPathing())
        {
            return;
        }
        ...
        if (OvaqMod.isBaritonePresent() && obstaclePasserConfig.getValue())
        {
    ...
    public void onPlayerMove(PlayerMoveEvent event)
    {
        ...
        {
            boolean boost = (mc.options.jumpKey.isPressed() || Modules.AUTO_WALK.isEnabled());
            if (boost)
            {
                applyVanillaBoost();
            }
    ...
    {
        if (mc.player == null || OvaqMod.isBaritonePresent() && BaritoneAPI.getProvider().getPrimaryBaritone().getPathingBehavior().isPathing())
        {
            return;
        }
    ...
    private boolean isBoostedByRocket()
    {
        for (Entity entity : mc.world.getEntities())
        {
            if (entity instanceof FireworkRocketEntity rocket
                    ...
                    && ((AccessorFireworkRocketEntity) rocket).getShooter() == mc.player)
            {
                return true;
            }
        }
