...
public class SpeedModule extends ToggleModule
{
    ...
    Config<Float> speedConfig = (new NumberConfig<>("Speed", "The speed for alternative modes", 0.1f, 4.0f, 50.0f, () -> speedModeConfig.getValue() == Speed.VANILLA));
    Config<Boolean> timerConfig = (new BooleanConfig("UseTimer", "Uses timer to increase acceleration", false, () -> isStrafe()));
    Config<Boolean> fastConfig = (new BooleanConfig("Fast", "Fast timer speed", false, () -> speedModeConfig.getValue() == Speed.STRAFE_STRICT && timerConfig.getValue()));
    ...
    public void onPlayerMove(PlayerMoveEvent event)
    {
        if (mc.player != null && mc.world != null && event.getType() == MovementType.SELF)
        {
            if (!MovementUtil.isInputtingMovement()
                    ...
                    || (Modules.FREECAM.isEnabled() && !Modules.FREECAM.control)
            ...
            else if (speedModeConfig.getValue() == Speed.STRAFE_STRICT)
            {
                ...
                if (timerConfig.getValue())
                {
                    if (fastConfig.getValue())
                    {
                        ...
                        Modules.TIMER.setTimer(Math.max(1.0f, res));
                    }
                    else
                    {
                        Modules.TIMER.setTimer(1.0888f);
