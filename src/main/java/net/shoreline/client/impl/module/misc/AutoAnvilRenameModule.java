...
import net.shoreline.client.mixin.accessor.AccessorAnvilScreen;
...
{
    Config<Selection> autoRenameSelectionConfig = (new EnumConfig<>("Selection", "The selection of items to rename", Selection.ALL, Selection.values()));
    Config<List<Item>> autoRenameWhitelistConfig = (new ItemListConfig<>("Whitelist", "The items to rename.", Items.SHULKER_BOX, Items.WHITE_SHULKER_BOX,
    ...
    Config<List<Item>> autoRenameBlacklistConfig = (new ItemListConfig<>("Blacklist", "The items to not rename.", Items.EXPERIENCE_BOTTLE));
    Config<String> autoRenameTextConfig = (new StringConfig("Text", "The text to rename the items to.", ""));
    Config<Integer> autoRenameDelayConfig = (new NumberConfig<>("Delay", "The delay between renaming items.", 0, 10, 20));
    Config<Boolean> debugConfig = (new BooleanConfig("Debug", "Prints debug information to chat.", false, () -> false));

    private final CacheTimer delayTimer = new CacheTimer();

    public AutoAnvilRenameModule()
    {
        super("AutoAnvilRename", "Automatically renames items in anvils.", ModuleCategory.MISC);
    }
    ...
    public void onTick(final TickEvent event)
    {
        ...
        for (int i = 3; i < 36 + 3; i++)
        {
            ...
            if (!itemStack.isEmpty() && !equalsName(itemStack, autoRenameTextConfig.getName()))
            {
                ...
                ((AccessorAnvilScreen) mc.currentScreen).getNameField().setText(name);
                if (debugConfig.getValue())
                {
