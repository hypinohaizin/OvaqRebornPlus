...
public class HoleESPModule extends ToggleModule
{
    ...
    Config<Boolean> outlineConfig = (new BooleanConfig("Outline", "Renders an outline around the hole", true));
    Config<Boolean> crossConfig = (new BooleanConfig("Cross", "Renders a cross on the floor of the hole", false));
    Config<Float> heightConfig = (new NumberConfig<>("Size", "Render height of holes", -1.0f, 1.00f, 1.0f));
    Config<Boolean> ignoreSelfConfig = (new BooleanConfig("IgnoreSelf", "Ignores the hole the player is standing in", false));
    ...
    Config<Color> obsidianConfig = (new ColorConfig("ObsidianColor", "The color for rendering obsidian holes", new Color(255, 0, 0, 80), () -> obsidianCheckConfig.getValue()));
    Config<Color> mixedConfig = (new ColorConfig("Obsidian-BedrockColor", "The color for rendering mixed holes", new Color(255, 255, 0, 80), () -> obsidianBedrockConfig.getValue()));
    Config<Color> bedrockConfig = (new ColorConfig("BedrockColor", "The color for rendering bedrock holes", new Color(0, 255, 0, 80)));
    Config<Color> voidColorConfig = (new ColorConfig("VoidColor", "The color for rendering bedrock holes", new Color(255, 0, 0, 140), () -> voidConfig.getValue()));
    Config<Integer> fadeTimeConfig = (new NumberConfig<>("Fade-Time", "Timer for the fade", 0, 300, 1000, () -> false));
    ...
    private void renderHole(MatrixStack matrixStack, Hole hole, int color1, int color2)
    {
        ...
        {
            RenderManager.renderBoundingBox(matrixStack, render, 2.0f, color2);
        }
    }
