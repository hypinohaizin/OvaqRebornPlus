...
public final class ServerModule extends ConcurrentModule
{
    ...
    Config<Boolean> illegalDisconnectConfig = (new BooleanConfig("IllegalDisconnect", "Disconnects by getting kicked from server", false));
    ...
    public void onPacketInbound(final PacketEvent.Inbound event)
    {
        if (event.getPacket() instanceof GameStateChangeS2CPacket packet)
        {
            ...
            {
                OvaqPlus.info("Server attempted to use Demo mode features on you!");
                ChatUtil.clientSendMessage("サーバーがあなたに対してデモモード機能を使用しようとしました！");
                event.cancel();
            }
        }
