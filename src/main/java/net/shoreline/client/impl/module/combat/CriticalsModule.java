...
public class CriticalsModule extends ToggleModule
{
    ...
    //
    // Config<Boolean> multitaskConfig = (new BooleanConfig("Multitask", "Allows crits when other combat modules are enabled", true));
    Config<CritMode> modeConfig = (new EnumConfig<>("Mode", "Mode for critical attack modifier", CritMode.PACKET, CritMode.values()));
    Config<Boolean> phaseOnlyConfig = (new BooleanConfig("PhasedOnly", "Only attempts criticals when phased", false, () -> modeConfig.getValue() == CritMode.GRIM_V3 || modeConfig.getValue() == CritMode.GRIM));
    Config<Boolean> wallsOnlyConfig = (new BooleanConfig("WallsOnly", "Only attempts criticals in walls", false, () -> (modeConfig.getValue() == CritMode.GRIM_V3 || modeConfig.getValue() == CritMode.GRIM) && phaseOnlyConfig.getValue()));
    Config<Boolean> moveFixConfig = (new BooleanConfig("MoveFix", "Pauses crits when moving", false, () -> modeConfig.getValue() == CritMode.GRIM_V3 || modeConfig.getValue() == CritMode.GRIM));
    //
