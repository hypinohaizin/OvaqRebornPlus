...
{
    Config<Double> speedConfig = (new NumberConfig<>("Speed", "The initial speed of the rocket.", 1.0, 1.5, 10.0));
    Config<Boolean> accelerateConfig = (new BooleanConfig("Accelerate", "Whether or not to accelerate the rocket.", false));
    Config<Double> accelSpeedConfig = (new NumberConfig<>("AccelSpeed", "The speed to accelerate the rocket at.", 0.1, 0.5, 5.0, () -> accelerateConfig.getValue()));
    Config<Double> maxSpeedConfig = (new NumberConfig<>("MaxSpeed", "The initial speed of the rocket.", 1.0, 3.0, 10.0, () -> accelerateConfig.getValue()));
    ...
    {
        if (event.getStage() != EventStage.PRE || mc.player.isInLava())
        {
            return;
        }
