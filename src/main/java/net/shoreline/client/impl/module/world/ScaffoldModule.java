...
public final class ScaffoldModule extends BlockPlacerModule
{
    ...
    Config<Boolean> rotateHoldConfig = (new BooleanConfig("RotateHold", "Holds rotations to scaffold blocks", false, () -> rotateConfig.getValue()));
    Config<Boolean> grimConfig = (new BooleanConfig("Grim", "Uses grim interactions", false));
    Config<Boolean> grimNewConfig = (new BooleanConfig("GrimV3", "Uses grim new interactions", false));
    Config<Boolean> keepYConfig = (new BooleanConfig("KeepY", "Keeps the same y-level", false));
    Config<Boolean> towerConfig = (new BooleanConfig("Tower", "Goes up faster when holding down space", true, () -> !grimNewConfig.getValue()));
    Config<BlockPicker> pickerConfig = (new EnumConfig<>("BlockSelection", "How to pick a block from the hotbar", BlockPicker.NORMAL, BlockPicker.values()));
        ...
        boolean result = Managers.INTERACT.placeBlock(blockData.getBlockPos(), slot, false, false, false, (state, angles) ->
        {
            if (rotateConfig.getValue())
            {
                ...
                if (state)
                {
                    ...
                    {
                        Managers.ROTATION.setRotationSilent2(rotations[0], rotations[1]);
                    }
                    else
                    {
