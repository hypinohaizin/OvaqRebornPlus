        ...
        if (mc.player != null && mc.world != null) {
            ...
            if (watermarkConfig.getValue()) {
                RenderManager.renderText(event.getContext(), String.format("%s §f%s (%s-%s)",
                        OvaqMod.MOD_NAME,  OvaqMod.MOD_VER,OvaqMod.MOD_BUILD_NUMBER, !BuildConfig.HASH.equals("null") ? "" + BuildConfig.HASH : ""), 2.0f, topLeft, getHudColor(rainbowOffset));

                topLeft += 10;
            }
            if (uidConfig.getValue()) {
                RenderManager.renderText(event.getContext(), String.format("UID §f%s", OvaqMod.UID), 2.0f, topLeft, getHudColor(rainbowOffset));
            }
