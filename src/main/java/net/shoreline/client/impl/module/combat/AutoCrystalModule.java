...
public class AutoCrystalModule extends CombatModule
{
    ...
    Config<Sequential> sequentialConfig = (new EnumConfig<>("Sequential", "Places a crystal after spawn", Sequential.NONE, Sequential.values()));
    Config<Boolean> idPredictConfig = (new BooleanConfig("BreakPredict", "Attempts to predict crystal entity ids", false));
    Config<Boolean> instantCalcConfig = (new BooleanConfig("Instant-Calc", "Calculates a crystal when it spawns and attacks if it meets MINIMUM requirements, this will result in non-ideal crystal attacks", false, () -> false));
    Config<Float> instantDamageConfig = (new NumberConfig<>("InstantDamage", "Minimum damage to attack crystals instantly", 1.0f, 6.0f, 10.0f, () -> false));
    Config<Boolean> instantMaxConfig = (new BooleanConfig("InstantMax", "Attacks crystals instantly if they exceed the previous max attack damage (Note: This is still not a perfect check because the next tick could have better damages)", true, () -> false));
    ...
    Config<Boolean> shulkersConfig = (new BooleanConfig("Shulkers", "Target shulker boxes", false));
    // BREAK SETTINGS
    ...
    Config<Float> attackLimitConfig = (new NumberConfig<>("AttackLimit", "The number of attacks before considering a crystal unbreakable", 0.5f, 1.5f, 20.0f));
    ...
    Config<ForcePlace> forcePlaceConfig = (new EnumConfig<>("PreventReplace", "Attempts to replace crystals in surrounds", ForcePlace.NONE, ForcePlace.values()));
    ...
    Config<Boolean> antiTotemConfig = (new BooleanConfig("Lethal-Totem", "Predicts totems and places crystals to instantly double pop and kill the target", false, () -> placeConfig.getValue()));
...
//            }
    ...
    private void placeInternal(BlockHitResult result, Hand hand)
    {
        ...
        {
            boolean flag = Modules.AUTO_XP.isEnabled() || mc.player.isUsingItem() && mc.player.getStackInHand(mc.player.getActiveHand()).getItem() instanceof ExperienceBottleItem;
