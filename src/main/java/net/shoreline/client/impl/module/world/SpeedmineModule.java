...
public class SpeedmineModule extends ToggleModule
{
    ...
    Config<Boolean> multitaskConfig = (new BooleanConfig("Multitask", "Allows mining while using items", false, () -> modeConfig.getValue() == SpeedmineMode.PACKET));
    Config<Boolean> doubleBreakConfig = (new BooleanConfig("DoubleBreak", "Allows you to mine two blocks at once", false, () -> modeConfig.getValue() == SpeedmineMode.PACKET));
    Config<Float> rangeConfig = (new NumberConfig<>("Range", "The range to mine blocks", 0.1f, 4.0f, 6.0f, () -> modeConfig.getValue() == SpeedmineMode.PACKET));
    Config<Float> speedConfig = (new NumberConfig<>("Speed", "The speed to mine blocks", 0.1f, 1.0f, 1.0f));
    Config<Boolean> instantConfig = (new BooleanConfig("Instant", "Instantly mines already broken blocks", false));
    ...
    Config<Boolean> switchResetConfig = (new BooleanConfig("SwitchReset", "Resets mining after switching items", false, () -> modeConfig.getValue() == SpeedmineMode.PACKET));
    Config<Boolean> grimConfig = (new BooleanConfig("Grim", "Uses grim block breaking speeds", false));
    Config<Boolean> grimNewConfig = (new BooleanConfig("GrimV3", "Uses new grim block breaking speeds", false, () -> grimConfig.getValue()));
    Config<Color> colorConfig = (new ColorConfig("MineColor", "The mine render color", Color.RED, false, false, () -> modeConfig.getValue() == SpeedmineMode.PACKET));
    Config<Color> colorDoneConfig = (new ColorConfig("DoneColor", "The done render color", Color.GREEN, false, false, () -> modeConfig.getValue() == SpeedmineMode.PACKET));
    Config<Integer> fadeTimeConfig = (new NumberConfig<>("Fade-Time", "Time to fade", 0, 250, 1000, () -> false));
    Config<Boolean> smoothColorConfig = (new BooleanConfig("ColorSmooth", "Interpolates from start to done color", false, () -> false));
    ...
    public void onPlayerTick(final TickEvent event)
    {
        ...
        if (Modules.AUTO_MINE.isEnabled())
        {
            return;
        }
    ...
    private void stopMining(MiningData data)
    {
        ...
        {
            RotationUtil.getRotationsTo(mc.player.getEyePos(), data.getPos().toCenterPos());
        }
