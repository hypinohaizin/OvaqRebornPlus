...
{
    Config<RenderMode> modeConfig = (new EnumConfig<>("Mode", "Render tracers to entities not visible on the screen", RenderMode.NORMAL, RenderMode.values()));
    Config<Target> targetConfig = (new EnumConfig<>("Target", "The body part of the entity to target", Target.FEET, Target.values()));
    Config<Float> widthConfig = (new NumberConfig<>("Width", "The line width of the tracer", 1.0f, 1.0f, 5.0f));
    ...
    //Config<Boolean> friendsConfig = (new BooleanConfig("Friends", "Render tracers to players that are friended", false, () -> playersConfig.getValue()));
    Config<Boolean> invisiblesConfig = (new BooleanConfig("Invisibles", "Render tracers to invisible entities", false));
    ...
    Config<Boolean> monstersConfig = (new BooleanConfig("Monsters", "Render tracers to monsters", false));
    ...
    Config<Boolean> animalsConfig = (new BooleanConfig("Animals", "Render tracers to animals", false));
    ...
    public void onRenderWorld(RenderWorldEvent event)
    {
        ...
        if (FreecamModule.getInstance().isEnabled())
        {
            ...
            Vec3d pos2 = Interpolation.getRenderPosition(pos1, Modules.FREECAM.getLastCameraPosition(), event.getTickDelta());
        ...
        for (Entity entity : mc.world.getEntities())
        {
            ...
            if (entity == null || !entity.isAlive() || entity == mc.player || !shouldDraw || entity instanceof PlayerEntity)
            {
                continue;
            }
    ...
    private Color getTracerColor(Entity entity)
    {
        ...
        else if (entity instanceof PlayerEntity && playersConfig.getValue())
        {
