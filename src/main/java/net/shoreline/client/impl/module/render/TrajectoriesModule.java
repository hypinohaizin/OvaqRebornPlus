...
import static net.shoreline.client.api.render.RenderManager.*;
        ...
        if (((throwItem instanceof BowItem || throwItem instanceof CrossbowItem || throwItem instanceof TridentItem) && mc.player.getItemUseTime() > 0) || isThrowableItem(throwItem))
        {
            ...
            while (!hasLanded)
            {
                ...
                for (Entity entity : mc.world.getOtherEntities(null, projectileBox))
                {
                    ...
                    renderBox(event.getMatrices(),
                            Interpolation.getInterpolatedEntityBox(entity), Modules.COLORS.getRGB(40));
                    renderBoundingBox(event.getMatrices(),
                            Interpolation.getInterpolatedEntityBox(entity), 1.5f, Modules.COLORS.getRGB(145));
                    landing = true;
                }
                ...
                if (result != null && result.getType() == HitResult.Type.BLOCK)
                {
                    ...
                    {
                        RenderManager.renderSide(event.getMatrices(), x - 0.6f, y - 0.6f, z, x + 0.6f, y + 0.6f, z, side, Modules.COLORS.getRGB(80));
                    }
                    else if (side == Direction.WEST || side == Direction.EAST)
                    {
                        RenderManager.       renderSide(event.getMatrices(), x, y - 0.6f, z - 0.6f, x, y + 0.6f, z + 0.6f, side, Modules.COLORS.getRGB(80));
                    }
                    else
                    {
                        RenderManager.renderSide(event.getMatrices(), x - 0.6f, y, z - 0.6f, x + 0.6f, y, z + 0.6f, side, Modules.COLORS.getRGB(80));
                    }
                }
                ...
                renderLine(event.getMatrices(), lastPos, pos, 1.5f, Modules.COLORS.getRGB());
                if (landing)
                {
                    hasLanded = true;
                }
