...
public final class SelfTrapModule extends ObsidianPlacerModule
{
    ...
    Config<Timing> timingConfig = (new EnumConfig<>("Timing", "Timing for replacing blocks", Timing.VANILLA, Timing.values()));
    Config<Boolean> prePlaceExplosionConfig = (new BooleanConfig("PrePlace-Explosions", "Pre places before explosions", false, () -> timingConfig.getValue() == Timing.SEQUENTIAL));
    Config<Boolean> prePlaceTickConfig = (new BooleanConfig("PrePlaceTick", "Pre places before ticks", false, () -> timingConfig.getValue() == Timing.SEQUENTIAL));
    ...
    Config<Boolean> mineExtendConfig = (new BooleanConfig("MineExtend", "Extends surround if the feet block is being mined", false));
    Config<Boolean> headExtendConfig = (new BooleanConfig("HeadExtend", "Extends surround if the head block is being mined", false));
    Config<Boolean> supportConfig = (new BooleanConfig("Support", "Creates a floor for the trap if there is none", false));
    Config<Boolean> headConfig = (new BooleanConfig("CoverHead", "Place a block at your head", false));
    Config<Integer> shiftTicksConfig = (new NumberConfig<>("ShiftTicks", "The number of blocks to place per tick", 1, 2, 10));
    Config<Float> shiftDelayConfig = (new NumberConfig<>("ShiftDelay", "The delay between each block placement interval", 0.0f, 1.0f, 5.0f));
    ...
    public List<BlockPos> getSurround(BlockPos playerPos, PlayerEntity player)
    {
        ...
        if (headConfig.getValue())
        {
            ...
            if (!Modules.AIR_INTERACT.isEnabled())
            {
