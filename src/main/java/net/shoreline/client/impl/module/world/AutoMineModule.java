...
import net.shoreline.client.api.module.CombatModule;
...
public class AutoMineModule extends CombatModule {
    ...
    Config<Boolean> avoidSelfConfig = (new BooleanConfig("AvoidSelf", "Avoids mining blocks in your surround", false, () -> autoConfig.getValue()));
    ...
    Config<Boolean> antiCrawlConfig = (new BooleanConfig("AntiCrawl", "Attempts to stop player from crawling", false));
    Config<Boolean> headConfig = (new BooleanConfig("TargetBody", "Attempts to mine players face blocks", false, () -> autoConfig.getValue()));
    Config<Boolean> aboveHeadConfig = (new BooleanConfig("TargetHead", "Attempts to mine above players head", false, () -> autoConfig.getValue()));
    Config<Boolean> doubleBreakConfig = (new BooleanConfig("DoubleBreak", "Allows you to mine two blocks at once", false));
    Config<Integer> mineTicksConfig = (new NumberConfig<>("MiningTicks", "The max number of ticks to hold a pickaxe for the packet mine", 5, 20, 60, () -> doubleBreakConfig.getValue()));
    Config<RemineMode> remineConfig = (new EnumConfig<>("Remine", "Remines already mined blocks", RemineMode.NORMAL, RemineMode.values()));
    Config<Boolean> packetInstantConfig = (new BooleanConfig("Fast", "Instant mines on packet", false, () -> remineConfig.getValue() == RemineMode.INSTANT));
    Config<Float> rangeConfig = (new NumberConfig<>("Range", "The range to mine blocks", 0.1f, 4.0f, 6.0f));
    ...
    Config<Boolean> swapBeforeConfig = (new BooleanConfig("SwapBefore", "Swaps before fully done mining", false, () -> swapConfig.getValue() != Swap.OFF));
    ...
    Config<Boolean> grimNewConfig = (new BooleanConfig("GrimV3", "Allows mining on new grim servers", false, () -> grimConfig.getValue()));
    ...
    Config<Boolean> smoothColorConfig = (new BooleanConfig("SmoothColor", "Interpolates from start to done color", false, () -> false));
    ...
    public void onTick(TickEvent event) {
        if (mc.player.isCreative() || mc.player.isSpectator() || event.getStage() != EventStage.PRE) {
            return;
        }
            ...
            if (instantMine.getBlockDamage() >= speedConfig.getValue()) {
                ...
                if (instantMine != null && (remineConfig.getValue() == RemineMode.INSTANT
                    ...
                    if (Modules.AUTO_CRYSTAL.isEnabled()
                            && Modules.AUTO_CRYSTAL.shouldPreForcePlace()) {
                        Modules.AUTO_CRYSTAL.placeCrystalForTarget(playerTarget, instantMine.getPos().down());
                    }
                ...
                if (antiCrawlConfig.getValue() && mc.player.getPose().equals(EntityPose.SWIMMING) && antiCrawlPos != null) {
                    ...
                    if (!isInstantMineComplete() && checkDataY(instantMine, targetPos, bedrockPhased)) {
                        ...
                        if (!phasedBlocks.isEmpty()) {
                            BlockPos pos1 = phasedBlocks.remove(0);
    ...
    private BlockPos getAntiCrawlPos(PlayerEntity playerTarget) {
        ...
        if (playerBelow) {
            mc.world.getBlockState(crawlingPos.down());
            return crawlingPos.down();
        } else {
            mc.world.getBlockState(crawlingPos.up());
    ...
    public static class MineData implements Comparable<MineData>
    {
        ...
        private double getPriority()
        {
            ...
            if (dist <= Modules.AUTO_CRYSTAL.getPlaceRange())
            {
                return 10.0f;
            }
