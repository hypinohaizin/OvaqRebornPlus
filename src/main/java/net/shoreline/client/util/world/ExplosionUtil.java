...
public class ExplosionUtil implements Globals {
     ...
     * @return
     */
    private static double getReduction(Entity entity, DamageSource source, double dmg, boolean assumeBestArmor) {
        ...
        if (entity instanceof LivingEntity living) {
            float armor = (float) Math.floor(living.getAttributeValue(net.minecraft.entity.attribute.EntityAttributes.GENERIC_ARMOR));
            float toughness = (float) living.getAttributeValue(net.minecraft.entity.attribute.EntityAttributes.GENERIC_ARMOR_TOUGHNESS);
            dmg = DamageUtil.getDamageLeft((float) dmg, armor, toughness);
            dmg = getResistanceReduction(living, dmg);
            dmg = getProtectionReduction(living, dmg, source, assumeBestArmor);
        }
    ...
    private static float getProtectionAmount(Iterable<ItemStack> equipment, boolean assumeBestArmor)
    {
        ...
        equipment.forEach(i ->
        {
            ...
            {
                int modifier = EnchantmentHelper.getLevel(Enchantments.BLAST_PROTECTION, i);
                int modifierBlast = EnchantmentHelper.getLevel(Enchantments.PROTECTION, i);
